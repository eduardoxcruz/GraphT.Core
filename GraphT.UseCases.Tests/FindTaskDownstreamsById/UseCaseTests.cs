...
public class UseCaseTests
{
	...
	public async Task Handle_WhenTaskHasDownstreams_ReturnsPagedListOfDownstreams()
	{
		...
		repository.FindAsync(Arg.Any<TaskIncludeDownstreamsSpecification>()).Returns(new PagedList<TodoTask>([ testTask ], 1, 1, 10));
	...
	public async Task Handle_ValidatesPagingParameters()
	{
		...
		unitOfWork.Repository<TodoTask>().Returns(repository);
		repository.FindByIdAsync(taskId).Returns(testTask);
		repository.FindAsync(Arg.Any<TaskIncludeDownstreamsSpecification>()).Returns(new PagedList<TodoTask>([ testTask ], 1, 2, 5));
